...
# Feature Flag: ENABLE_SIGNAL=False 면 로드 자체를 건너뜀
# 기본 True — 모델은 항상 로드해두고, 켜고 끄기는 앱(SharedPreferences)에서 제어
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ENABLE_SIGNAL = True   # 클라이언트(앱)가 토글로 호출 여부 결정
...
if ENABLE_SIGNAL:
    ...
    print("[SafeStep] 신호등 기능 비활성화 (ENABLE_SIGNAL=False)")
...
async def signal(file: UploadFile = File(...)):
    ...
    """
    # Feature Flag: 신호등 비활성화 시 즉시 종료
    if not ENABLE_SIGNAL:
        return JSONResponse({"color": None, "confidence": None, "disabled": True})
    try:
        raw   = await file.read()
        image = Image.open(io.BytesIO(raw)).convert("RGB")
    except Exception as e:
